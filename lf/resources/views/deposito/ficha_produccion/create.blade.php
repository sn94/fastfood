...
<style>
    ...
    table.PRODUCCION thead tr,    table.PRODUCCION tbody tr {
        display: grid;
...
<h2 class="text-center mt-2" style="font-family: titlefont;">Ficha de producción</h2>
...
<div class="row m-2">
    <div class="col-12 col-md-12">
        ...
        @if( $PRODUCCION_ID != '' )
        <form id="PRODUCCIONFORM" action="<?= url("deposito/ficha-produccion") ?>" method="POST"  onkeypress="if(event.keyCode == 13) event.preventDefault();"    onsubmit="guardar(event)">
            <input type="hidden" name="_method" value="PUT">
            @else
            <form action="<?= url("deposito/ficha-produccion") ?>" method="POST"  onkeypress="if(event.keyCode == 13) event.preventDefault();"    onsubmit="guardar(event)">
                @endif
                ...
                @include("deposito.ficha_produccion.header")

                <div class="row bg-dark">
                    <div class="col-12 col-md-6">
                        @include("deposito.ficha_produccion.grill_productos")
                    </div>
                    <div class="col-12 col-md-6">
                        @include("deposito.ficha_produccion.grill_ingredientes")
                    </div>

                    <div class="col-12 col-md-6 mt-1">
                        @include("deposito.ficha_produccion.grill_insumos")
                    </div>
                </div>
...
@include("deposito.ficha_produccion.cargador_tabla")
...
*/
    ...
    async function restaurar_ficha_produccion() {
        ...
        if (produccionId != "") {
            ...
            let url__ = "<?= url('deposito/ficha-produccion') ?>/" + produccionId;
