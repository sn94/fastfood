...
class SesionesController extends Controller
{
    ...
    public function  totalesArqueo($SESION,  $ARRAY_RETURN = FALSE)
    {
        ...
        $totales =    Ventas::join("ventas_det", "ventas_det.VENTA_ID", "=", "ventas.REGNRO")
            ...
            ->select(
                DB::raw("sum(ventas_det.CANTIDAD*ventas_det.P_UNITARIO) AS TOTAL"),
                DB::raw("if(servicios.COSTO IS NULL, 0, servicios.COSTO) AS COSTO_SERVICIO"),
                "FORMA"
            )
            ->groupBy("FORMA")
    ...
    public function cerrar($SESIONID =  NULL)
    {
        ...
        if (request()->getMethod() == "GET") {
            ...
            try {
                ...
                if ( Config::get("app.my_config.funciones.envio_email_post_arqueo_caja") == "S") {
                    try {
                        $this->enviarArqueoPorEmail($datosDelArqueo);
                        return response()->json(['ok' =>  "Sesión cerrada"]);
                    } catch (Exception $ex) {
                        return response()->json(['ok' =>  "La sesion ha sido cerrada correctamente."]);
                        // Pero no se ha podido enviar el email correspondiente. Verifique el funcionamiento de correo.
                    }
                }else   return response()->json(['ok' =>  "Sesión cerrada"]);
