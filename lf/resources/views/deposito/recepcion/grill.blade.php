...
<script>
    ...
    function deleteme(esto) {
        ...
        //quitar del modelo 
        recepcion_model = recepcion_model.filter(function(arg) {
    ...
    function actualizar_fila(objec) {
        let existe = recepcion_model.map((ar) => ar.ITEM).filter((ar) => parseInt(ar) == parseInt(objec.ITEM)).length;
        if (existe > 0) {
            let numero_de_filas = $("#RECEPCION-DETALLE tr").length;
            if (numero_de_filas == 0) return false;

            for (let nf = 0; nf < numero_de_filas; nf++) {
                let lafila = $("#RECEPCION-DETALLE tr")[nf];
    ...
    function actualiza_modelo_de_datos(NuevoObjeto) {
        ...
        let EXISTE = recepcion_model.filter(
            (ar) => {
        ...
        if (EXISTE == 0) {
            recepcion_model.push(NuevoObjeto);
            return;
        }

        recepcion_model = recepcion_model.map(
            (ar) => {
    ...
    function cargar_tabla() {
        ...
        let modeloDeItemSeleccionado = recepcion_model.filter(
            (ar) => {
        ...
        if (regnro != "") {
            ...
            $("#RECEPCION-DETALLE").append(nueva_tr);
