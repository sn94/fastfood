...
class VentasController extends Controller
{
    ...
    public function ticket($IDVENTA)
    {
        ...
        $formato =  request()->header("formato");
        $total_copias  =  request()->header("total-copias"); 
        $total_copias_r  =  request()->header("total-copias-r"); 
        ...
        if ($formato ==  "email") {
            ...
            if ($venta->DELIVERY == "S")
                ...
                $ejemplar="";
            if( $total_copias == $total_copias_r  )
            $ejemplar = "o"; 
            else $ejemplar = "c"; 
         
            return view(
                "ventas.proceso.ticket.version_impresa",
                ["VENTA" => $venta, "DETALLE" => $detalle, "DELIVERY" =>  $delivery, "EJEMPLAR"=> $ejemplar ]
            );
        }
    }
