...
function createWorkBook(datos, whois) {
  if (whois == undefined) whois = "Clientes"; ///$("#mensajes").html("Creando archivo xls/xlsx...");
  ...
  var ws = XLSX.utils.aoa_to_sheet(ws_data); //var ws = XLSX.utils.table_to_sheet(document.getElementById('tabla'));
...
async function callToXlsGen_post_url(url, titulo, formpost) {
  ...
  let req = await fetch(rutaxls, {
    method: "POST",
    body: $(formpost).serialize()
  });
