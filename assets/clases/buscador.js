...
class DataSearcher {
    ...
    async formatoPdf(ev) {
        ...
        //Filesaver Js Lib
        let blobData = new Blob([respBlob], { type: "application/pdf;charset=utf-8" });
    ...
    prepararParametros() {
        if (this.requestContentType == 'application/x-www-form-urlencoded')
            return Object.entries(this.parametros).map(([clave, valor]) => clave + "=" + valor).join("&");
        if (this.requestContentType == 'application/json')
            return JSON.stringify(this.parametros);
        else "";
    ...
    async buscar() {
        ...
        let metodoReq = this.metodo;
        //parametros
        let params = this.prepararParametros();
        //Send
        let requestSetting = {
